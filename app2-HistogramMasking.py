...
def extract_yellow_histogram_features(image, bins=10):
    ...
    # Masking (exclude iris and skin)
    iris_mask = L < 70
    skin_mask = (A > 130) & (B > 130)
    valid_mask = ~(iris_mask | skin_mask)

    # Extract valid sclera pixels
    L_valid, A_valid, B_valid = L[valid_mask], A[valid_mask], B[valid_mask]

    # If no valid pixels, return zero vector
    if L_valid.size == 0:
        return np.zeros(bins * 3)

    # Compute histograms for valid sclera pixels
...
def compute_way_kmeans_lab(image, k=6):
    ...
    # Compute Weighted Average Yellowness (WAY)
...
def predict():
    ...
    # If jaundice is detected, compute WAY and severity
